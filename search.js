...
function getArray(str){
  ...
  function addToCart(ele) {
    ...
    for (let i = 0; i < final.length; i++) {
      if (final[i].name == ele.name) {
        final[i].quantity++
        final[i].cartprice = ele.price * final[i].quantity
        flag = true
        alert(`Product ${ele.name} Added to Cart ${final[i].quantity} Times`)
      }
    }
    if (!flag) {
      ele.quantity = 1
    ...
    localStorage.setItem('cartData', JSON.stringify(final))
  }
  
  function handlesortprice() {
    let selected = document.getElementById('sort_price').value
    if (selected == 'ascending') {
      let filterlist = final.sort(function (a, b) {
  ...
  function handlesortbrand() {
    ...
    let filterlist = final.filter(function (ele) {
  ...
  gender.forEach(function (ele) {
    ele.addEventListener('change', function () {
      if (event.target.checked == true) {
        let filteredlist = final.filter(function (kid) {
